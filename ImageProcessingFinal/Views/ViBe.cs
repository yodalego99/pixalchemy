...
public class ViBe
{
    private byte[,,,] _compareFrames; // Two consequent frames
    private byte[,,] _frameImageBytes;
    private int _matchCount; // Number of matches
    ...
    private int GetRandomNeighbourPixel(int coord)
    {
        ...
        if (coord == FrameImage.Size.Height - 1 || coord == FrameImage.Size.Width - 1 || coord == 0) return coord;
